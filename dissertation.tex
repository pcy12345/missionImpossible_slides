...
The proliferation of networked systems across critical infrastructure, cloud computing, and Internet of Things (IoT) environments has dramatically expanded the attack surface available to adversaries. According to recent industry reports, the global cost of cybercrime is projected to exceed \$10.5 trillion annually by 2025, with network intrusions constituting one of the most prevalent and damaging attack vectors. Network Intrusion Detection Systems (NIDS) serve as a foundational defense layer, monitoring network traffic flows and classifying them as benign or malicious in real time.

Historically, NIDS have followed two distinct paradigms. \emph{Signature-based} systems, exemplified by Snort and Suricata, match observed traffic against a database of known attack patterns. While highly precise for known threats, these systems are fundamentally unable to detect novel attacks, which is a significant limitation given that adversaries continuously develop new techniques, tactics, and procedures (TTPs). \emph{Anomaly-based} systems, by contrast, learn a model of normal behavior and flag deviations. Machine learning approaches have dominated this paradigm for over a decade, with Random Forests, Support Vector Machines, and deep neural networks achieving F1 scores exceeding 0.99 on standard benchmarks such as UNSW-NB15 and CIC-IDS2017.
...
\begin{enumerate}[label=(\arabic*)]
    \item \textbf{Opacity.} Black-box classifiers produce predictions without explanations. When a classifier labels a flow as ``attack,'' the security analyst cannot determine \emph{why}, that is, which features contributed, which rules were triggered, or whether the decision is consistent with organizational security policy. This opacity undermines trust and impedes regulatory compliance, particularly in sectors governed by frameworks such as NIST SP 800-53 and the EU AI Act.

    \item \textbf{Static decision boundaries.} Supervised classifiers are trained on a fixed dataset and deployed with frozen parameters. When adversaries introduce novel attack types (zero-day exploits, new malware families, evolved command-and-control (C2) protocols), the classifier cannot adapt without costly offline retraining, data re-collection, and model re-validation.

    \item \textbf{Absence of formal safety guarantees.} No mainstream ML-based IDS provides a mechanism to \emph{prove} that its decisions satisfy a set of safety constraints. A classifier might achieve 99\% accuracy overall yet systematically misclassify a specific attack variant that happens to fall near the decision boundary. Without formal verification, such failure modes are undetectable until exploitation occurs.
\end{enumerate}

This dissertation addresses all three challenges through the \ASRRL{} framework, which integrates symbolic reasoning, reinforcement learning, and unsupervised clustering into a unified pipeline that is simultaneously adaptive, explainable, and formally verifiable.
...
\begin{enumerate}[label=\textbf{RQ\arabic*.}]
    \item \textbf{Accuracy.} Does the \ASRRL{} framework achieve detection accuracy (measured by F1 score, precision, recall, FPR, and FNR) that is statistically comparable to or superior to established baseline classifiers (Random Forest, XGBoost, LightGBM, SVM, KNN, Naive Bayes, MLP) across multiple benchmark datasets?

    \item \textbf{Formal Verifiability.} Can Z3 SMT constraints extracted from decision tree paths and DBSCAN-detected novel patterns provide meaningful formal verification of reinforcement learning actions, as measured by constraint coverage, decision traceability, and the Composite Verifiability Score (CVS)?

    \item \textbf{Novel Attack Adaptation.} Does the online DBSCAN-based novel pattern detector enable \ASRRL{} to adapt to previously unseen attack types during deployment, and does this adaptation measurably improve detection performance relative to static baselines in a temporally phased evaluation?

    \item \textbf{Robustness.} How does \ASRRL{} perform under adversarial feature perturbations and simulated concept drift compared to baseline classifiers?

    \item \textbf{Operational Readiness.} Does the dynamic buffer and threshold mechanism enable \ASRRL{} to maintain stable performance across varying attack intensities without manual parameter tuning?
\end{enumerate}
...
\begin{enumerate}[label=\textbf{O\arabic*.}]
    \item Design a nine-stage classification pipeline that integrates decision trees, Z3 SMT constraints, Q-learning, and DBSCAN clustering.
    ...
    \item Evaluate \ASRRL{} against seven baseline classifiers across four benchmark datasets using nine evaluation dimensions.
...
\begin{enumerate}
    \item \textbf{A Novel Nine-Stage Formally Verifiable IDS Architecture.} \ASRRL{} is, to the best of our knowledge, the first IDS framework that integrates decision tree classification, Z3 SMT constraint verification, tabular Q-learning, and DBSCAN novel pattern detection into a single, formally verifiable pipeline. Every classification action is verified against a formally derived constraint set before execution, achieving a Composite Verifiability Score of 0.960, compared to 0.16--0.24 for all baseline classifiers.
...
A network flow is a sequence of packets sharing a common 5-tuple: source IP, destination IP, source port, destination port, and transport protocol. Flows are characterized by aggregate statistics including duration, packet rate, byte rate, and inter-arrival time variability.
\end{definition}

\begin{definition}[Symbolic Safety Shield]
A formal verification mechanism that constrains the reinforcement learning agent's action space by encoding decision tree paths and DBSCAN-detected novel patterns as Z3 SMT constraints. Before any classification action is executed, the shield verifies that the action satisfies all constraints; if verification fails, the shield overrides the agent's decision with a safe alternative.
\end{definition}

\begin{definition}[Composite Verifiability Score (CVS)]
A scalar metric $\text{CVS} \in [0, 1]$ computed as the arithmetic mean of seven sub-metrics: formal rule extractability, constraint coverage, decision traceability, safety guarantee, deterministic reproducibility, audit trail completeness, and explanation complexity. A CVS $\geq 0.70$ indicates deployment readiness for critical infrastructure.
\end{definition}

\begin{definition}[Novel Attack Pattern]
A previously unseen cluster of misclassified network flows discovered by DBSCAN during online operation. Each novel pattern represents an attack variant not present in the training data. Upon discovery, the cluster center is converted to a Z3 \texttt{BLOCK} constraint, enabling immediate adaptation without retraining.
\end{definition}

\begin{definition}[Concept Drift]
A change in the statistical properties of the target variable over time. In the IDS context, concept drift occurs when adversaries modify their TTPs, causing the distribution of attack features to shift relative to the training distribution.
\end{definition}
...
\begin{itemize}
    \item \textbf{Chapter~\ref{ch:literature}} provides a comprehensive review and critical synthesis of the literature on machine learning-based intrusion detection, reinforcement learning for security, symbolic reasoning and formal verification, online learning for novel attack detection, and explainable AI in cybersecurity.

    \item \textbf{Chapter~\ref{ch:methodology}} details the \ASRRL{} framework design, including the nine-stage pipeline, all algorithms in formal notation, the reward structure, the novel attack detection mechanism, the Composite Verifiability Score, and the experimental methodology.

    \item \textbf{Chapter~\ref{ch:findings}} presents the experimental results across nine evaluation dimensions, four datasets, and seven baseline comparisons, with statistical significance analysis and discussion.
...
LIME (Local Interpretable Model-agnostic Explanations), introduced by Ribeiro et al.~\citep{ribeiro2016}, generates local explanations by fitting an interpretable surrogate model (typically a sparse linear model) to the neighborhood of a prediction. LIME perturbs the input features, queries the black-box model on the perturbed instances, and trains the surrogate to approximate the model's local behavior. In cybersecurity, Marino et al.~\citep{marino2018} applied LIME to explain Random Forest IDS predictions on NSL-KDD, finding that the top-3 explanatory features (service type, flag, and source bytes) were consistent with domain knowledge. However, they also observed that LIME's explanations were unstable, as running LIME twice on the same prediction could produce different feature importance rankings due to the stochastic perturbation process.

SHAP (SHapley Additive exPlanations), proposed by Lundberg and Lee~\citep{lundberg2017}, provides a more theoretically grounded explanation framework based on cooperative game theory. SHAP assigns each feature a Shapley value representing its marginal contribution to the prediction, with the property that Shapley values are the unique attribution method satisfying local accuracy, missingness, and consistency axioms. In IDS applications, SHAP has been used to identify the most influential features in malware detection and to validate that classifiers are not relying on spurious correlations.

However, post-hoc explanations have fundamental limitations that are particularly concerning in security-critical settings. Slack et al.~\citep{slack2020} demonstrated that LIME and SHAP explanations can be manipulated by adversaries who construct models that behave differently in the neighborhood queried by the explanation method than in the broader input space. Their ``fooling LIME and SHAP'' attack showed that a biased classifier could produce explanations indistinguishable from those of an unbiased classifier, undermining the trustworthiness of post-hoc explanations. Molnar~\citep{molnar2020} argued that this vulnerability is inherent to post-hoc methods: because they \emph{approximate} the model's behavior rather than \emph{constituting} it, there is no formal guarantee that the explanation faithfully represents the actual decision process.

\subsection{Inherently Interpretable Models}

Decision trees and rule-based systems are inherently interpretable, as each prediction can be traced to a specific path of feature comparisons. Molnar~\citep{molnar2020} argued that inherently interpretable models should be preferred over post-hoc explanations when the stakes are high, because their explanations are guaranteed to be faithful because the explanation \emph{is} the decision process, not an approximation of it. In the IDS domain, Buczak and Guven~\citep{buczak2016} surveyed 35 rule-based IDS approaches published between 2000 and 2015, noting that while rule-based systems provide excellent interpretability, they consistently underperform ML-based methods on modern datasets due to their inability to learn complex feature interactions and their reliance on manually authored rules that cannot adapt to evolving threats.

Lin et al.~\citep{lin2022} proposed a rule-extraction approach that trains a neural network classifier and then distills its knowledge into a set of human-readable rules using the DeepRED algorithm. Their approach achieved 95.3\% accuracy on NSL-KDD with a rule set containing 47 rules, each involving 3--5 feature conditions. While this approach provides interpretable rules with competitive accuracy, the rules are static after extraction and cannot adapt to novel attacks without retraining the underlying neural network and re-extracting the rules.

The \ASRRL{} framework combines the interpretability of decision trees with the adaptability of RL and the formal guarantees of SMT solving. This combination achieves a form of interpretability that goes beyond both post-hoc explanations and static rule systems. Because \ASRRL{}'s explanations are derived directly from the Z3 constraints that \emph{actually govern} the system's decisions (rather than from post-hoc approximation methods), they are provably faithful to the decision process and immune to the manipulation attacks demonstrated by Slack et al.~\citep{slack2020}.
...
The literature reveals a clear gap: no existing IDS framework provides simultaneous high accuracy, formal verifiability, online adaptability, and novel attack detection. The \ASRRL{} framework is designed to fill this gap by integrating complementary techniques into a unified, nine-stage pipeline.
...
\textbf{Safe and Shielded RL.} Approaches such as SafeRL-IDS~\citep{wang2022saferl} and Shield-RL~\citep{kumar2023shield} address unsafe exploration through action filtering or constrained optimization. However, SafeRL-IDS achieves safety without interpretability (F1~=~0.79, no decision traceability), while Shield-RL provides only partial formal guarantees because its shielding mechanism is not grounded in a formally verified constraint set derived from the data.

\textbf{Neurosymbolic Approaches.} DeepSymNet~\citep{zhang2021deepsymnet} and LogicIDS~\citep{li2021logicids} combine symbolic reasoning with neural components but lack runtime adaptation. DeepSymNet extracts post-hoc symbolic rules from trained networks (F1~=~0.75) but cannot update its rules without complete retraining. LogicIDS provides partial formal guarantees but its constraint set is static and cannot incorporate novel attack patterns discovered after deployment.

\textbf{Our Prior Work (Young and Ji, 2025).} Our previously published framework~\citep{young2025adaptive} demonstrated the feasibility of integrating Z3 SMT constraint verification with Q-learning for network traffic classification, achieving F1~=~0.773 with 33\% false negative reduction and 1,247 shield activations during training. However, that work exhibited several limitations that the present dissertation directly addresses:
\begin{itemize}
    ...
    \item \textbf{Lower absolute accuracy.} F1 of 0.773 was competitive with but below ensemble baselines (XGBoost F1~=~0.841), reflecting the early-stage architecture's inability to fully leverage the complementary strengths of symbolic and statistical components.
\end{itemize}
...
Young \& Ji (2025) & Static constraints, no DBSCAN, no dynamic buffer, limited coverage & Nine-stage pipeline with all five contributions \\
\bottomrule
\end{tabular}
\end{table}
...
This chapter presents the complete design of the \ASRRL{} framework, beginning with an overview of the nine-stage pipeline (\S\ref{sec:pipeline_overview}), followed by detailed algorithmic descriptions of each component (\S\ref{sec:decision_tree}--\S\ref{sec:dynamic_buffer}), the formal definitions of all evaluation metrics (\S\ref{sec:eval_metrics}), and the experimental design (\S\ref{sec:experimental_design}).


\section{Pipeline Overview}
\label{sec:pipeline_overview}

The \ASRRL{} framework processes each network flow through a nine-stage pipeline, illustrated in Figure~\ref{fig:pipeline}.
...
\end{tikzpicture}
\caption{The nine-stage \ASRRL{} classification pipeline. Dashed arrows indicate the feedback loop where misclassified flows detected by DBSCAN generate new Z3 constraints.}
\label{fig:pipeline}
\end{figure}

\noindent The nine stages are:
...
\noindent The pipeline's nine-stage architecture is motivated by the principle of \emph{defense in depth}: no single component is solely responsible for detection accuracy or safety. The decision tree provides a fast, interpretable base classifier; the Z3 constraint set provides formal guarantees that no action violates learned safety invariants; the DBSCAN detector provides adaptability to novel threats; and the RL agent provides online policy optimization that improves over time. Each stage's output feeds into subsequent stages, creating a layered verification pipeline where errors at one stage can be caught and corrected by downstream stages.
...
\end{equation}
This implication-based formulation is critical because it allows the Z3 solver to determine not just \emph{whether} a constraint is satisfied, but \emph{what action} should be taken when it is satisfied.
...
\begin{algorithmic}[1]
\Require Trained decision tree $T$ with nodes $\mathcal{N}$, features $\{x_0, \ldots, x_9\}$
...
\EndFor
\State solver.add($\bigwedge_{i=0}^{9} x_i = \bx[i]$) \Comment{Assert observed values}
...
When the buffer reaches capacity, the oldest entry is evicted (FIFO).
...
\phi_c^{\leq} = \bigwedge_{i=0}^{9} (x_i \leq (1+\delta) \cdot \mu_c[i]) \implies \texttt{BLOCK}
\end{equation}
\begin{equation}
\phi_c^{>} = \bigwedge_{i=0}^{9} (x_i > (1-\delta) \cdot \mu_c[i]) \implies \texttt{BLOCK}
\end{equation}
...
\For{$e = 0$ \textbf{to} $E-1$}
    ...
    \If{$e \bmod k = 0$ \textbf{and} $e > 0$} \Comment{Periodic novel detection}
        ...
        \For{each $\mu_c \in \mathcal{C}_e$}
            \State $\text{path}_{\leq} \leftarrow \{(i, (1+\delta) \cdot \mu_c[i], \leq) \mid i = 0,\ldots,9\}$
            \State $\text{path}_{>} \leftarrow \{(i, (1-\delta) \cdot \mu_c[i], >) \mid i = 0,\ldots,9\}$
...
To address this gap, this dissertation introduces the Composite Verifiability Score (CVS), a seven-dimensional metric that quantifies the formal auditability of an IDS. The CVS is computed as the unweighted arithmetic mean of seven sub-metrics, each normalized to $[0, 1]$:
...
The equal weighting reflects the principle that each dimension represents a \emph{necessary} (though not independently sufficient) property for full formal verifiability. An IDS that achieves perfect scores on six dimensions but zero on one (e.g., excellent accuracy but no audit trail) is not fully verifiable. The seven sub-metrics are defined as follows:
...
\textbf{Applicability to baseline models.} The CVS is designed to be applied uniformly to any IDS approach, not just \ASRRL{}. For black-box models (Random Forest, XGBoost, neural networks), $v_1$ through $v_4$ typically score 0.0 because these models do not produce formal constraints, cannot provide constraint coverage, lack complete decision traceability, and offer no pre-execution safety verification. They may score moderately on $v_5$ (reproducibility, if deterministic) and $v_6$ (audit completeness, if logging is implemented), and variably on $v_7$ (explanation complexity, if post-hoc methods are used). This systematic evaluation makes explicit the verifiability deficit that motivates the \ASRRL{} architecture.


\section{Datasets}
\label{sec:datasets}

Four benchmark datasets are used for evaluation:
...
Ten independent trials with different random seeds per dataset. Each trial trains \ASRRL{} and all seven baselines on a 70/30 stratified train/test split. Reports mean $\pm$ standard deviation for all metrics.
...
\begin{table}[htbp]
\centering
\caption{Multi-trial F1 scores (mean $\pm$ std) across four datasets. Best result per dataset in bold.}
...
\midrule
\ASRRL{} & \textbf{0.9963 $\pm$ 0.0009} & 0.9453 $\pm$ 0.0038 & 0.9270 $\pm$ 0.0052 & 0.9580 \\
Random Forest & 0.9942 $\pm$ 0.0012 & \textbf{0.9563 $\pm$ 0.0028} & \textbf{0.9498 $\pm$ 0.0041} & 0.9730 \\
XGBoost & 0.9955 $\pm$ 0.0008 & 0.9548 $\pm$ 0.0031 & 0.9476 $\pm$ 0.0035 & 0.9740 \\
LightGBM & 0.9951 $\pm$ 0.0010 & 0.9539 $\pm$ 0.0033 & 0.9461 $\pm$ 0.0039 & --- \\
...
\ASRRL{} achieves the highest F1 on CSE-CIC-IDS-2018 ($0.9963$), demonstrating competitive performance with ensemble methods. On UNSW-NB15 and CIC-IDS2017, \ASRRL{} ranks within 1--2\% of the top ensemble baselines, which is notable given that \ASRRL{} trades marginal accuracy for formal verifiability, a capability that no baseline provides (see Section~\ref{sec:res_verifiability}). On MISP ThreatFox (scaled to 2,136,408 flows), \ASRRL{} achieves $0.958$, slightly below the ensemble baselines (RF $0.973$, XGB $0.974$) that benefit from the ThreatFox data's structured nature.

The performance gap between \ASRRL{} and ensemble methods on UNSW-NB15 and CIC-IDS2017 is attributable to the decision tree's depth constraint, which limits the expressiveness of the base classifier. Ensemble methods aggregate hundreds of trees, each capturing different aspects of the feature space, whereas \ASRRL{} uses a single tree whose depth is deliberately bounded to keep the Z3 constraint set tractable. Despite this architectural trade-off, \ASRRL{} remains within 2\% of the best ensemble performance while providing formal safety guarantees that ensemble methods cannot offer.
...
\caption{ASRRL vs.\ black-box baselines: comprehensive performance comparison across all metrics and datasets.}
...
\midrule
Random Forest & 0.0039 & 0.0028 & Yes \\
XGBoost & 0.0840 & 0.0890 & No \\
LightGBM & 0.0195 & 0.0156 & Yes \\
...
\ASRRL{} shows statistically significant improvement over 6 of 7 baselines at $p < 0.05$. The comparison with XGBoost is not statistically significant ($p = 0.084$), indicating that the two methods achieve comparable F1 on this dataset, but XGBoost lacks verifiability (CVS = 0.24 vs.\ \ASRRL{}'s 0.96).
...
Random Forest & 0.00 & 0.00 & 0.00 & 0.00 & 1.00 & 0.30 & 0.35 & 0.236 \\
XGBoost & 0.00 & 0.00 & 0.00 & 0.00 & 1.00 & 0.30 & 0.38 & 0.240 \\
...
\ASRRL{} achieves a CVS of $0.960$, exceeding the deployment threshold of $0.70$ by a substantial margin. All baselines score below $0.25$, as none provides formal rule extraction, constraint coverage, decision traceability, or safety guarantees. The only sub-metric where baselines score competitively is deterministic reproducibility ($v_5 = 1.0$ for tree-based methods).
...
Random Forest & 0.9998 & 0.9401 & 0.9187 & 0.9730 \\
XGBoost & 0.9998 & 0.9389 & 0.9165 & 0.9740 \\
...
The MISP ThreatFox experiment provides particularly compelling results because the holdout attack types (RAT, stealer, loader, ransomware) represent real malware families from the ThreatFox feed:
\begin{itemize}
...
Random Forest & 0.994 & 0.991 & 0.974 & 0.951 & 0.912 & 0.878 & 0.823 \\
XGBoost & 0.996 & 0.993 & 0.981 & 0.962 & 0.929 & 0.897 & 0.841 \\
\bottomrule
\end{tabular}
\end{table}
...
In contrast, ensemble baselines (Random Forest, XGBoost) degrade gracefully because their aggregation of many independent trees provides natural averaging that smooths out small perturbations. However, this robustness comes at the cost of formal verifiability, as the ensemble's decision process cannot be expressed as a tractable set of Z3 constraints.
...
\section{Comparison with Prior Work (Young and Ji, 2025)}
\label{sec:prior_comparison}

This section presents a direct empirical comparison between the \ASRRL{} framework introduced in this dissertation and our previously published approach~\citep{young2025adaptive}, which integrated Z3 SMT constraint verification with Q-learning for adaptive network traffic classification. The prior work was evaluated on UNSW-NB15, CIC-IDS2017, and KDD~Cup~99 datasets and represents the first iteration of the symbolic-RL IDS concept. The comparison quantifies the improvements achieved by the five contributions of the present work.

\subsection{Architecture Differences}

The prior framework employed a four-stage pipeline: (1)~feature engineering with 42 features, (2)~decision tree training (max depth~6, min samples~15), (3)~Z3 constraint extraction, and (4)~CMDP agent with symbolic safety shield. The present \ASRRL{} framework extends this to a nine-stage pipeline by adding: (5)~DBSCAN-based novel attack detection, (6)~automatic Z3 constraint injection from discovered patterns, (7)~dynamic buffer sizing, (8)~adaptive threshold adjustment, and (9)~comprehensive verifiability scoring via CVS.
...
\begin{table}[htbp]
\centering
\caption{Comparison of the prior framework (Young and Ji, 2025) with \ASRRL{} (this dissertation).}
\label{tab:prior_vs_asrrl}
\small
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{Young \& Ji (2025)} & \textbf{ASRRL (this work)} \\
\midrule
...
\midrule
RQ1 & \ASRRL{} achieves F1 of 0.9963 on CSE (best among all models), and competitive F1 within 1--2\% of ensemble baselines on UNSW, CIC2017, and MISP. Statistical significance confirmed for 6/7 baselines. \\
RQ2 & \ASRRL{} achieves CVS of 0.960, vs.\ 0.16--0.24 for baselines. Z3 provides 100\% constraint coverage, 100\% fidelity, and 100\% safety guarantee. \\
...
This dissertation presented the Adaptive Symbolic Reasoning and Reinforcement Learning (\ASRRL{}) framework for network intrusion detection. This framework is a nine-stage pipeline that unifies decision tree classification, Z3 SMT constraint verification, tabular Q-learning, DBSCAN-based novel attack discovery, and adaptive buffer/threshold management into a single, formally verifiable system. The principal contributions of this work are:

\begin{enumerate}
    \item \textbf{A Novel Nine-Stage Formally Verifiable IDS Architecture.} This dissertation introduces the first IDS framework that integrates symbolic reasoning (Z3 SMT solver) with reinforcement learning under a formal safety shield. Unlike prior work that treats ML classifiers as opaque decision-makers, \ASRRL{} guarantees that every classification action is verified against a formally derived constraint set before execution. The resulting system achieves a Composite Verifiability Score (CVS) of 0.960, a metric also introduced in this work, compared to 0.16--0.24 for seven baseline classifiers, establishing a new standard for auditable intrusion detection.
...
\begin{itemize}
    ...
    \item \textbf{Zero-day resilience.} The online DBSCAN detector reduces the window of vulnerability between zero-day emergence and detection, without requiring costly retraining cycles.
\end{itemize}

\section{Future Work}

Three concrete directions for future research emerge from this work, each achievable within a one- to two-semester research effort:
...
\bibitem[Young and Ji(2025)]{young2025adaptive}
Young, P. and Ji, S.-Y. (2025).
Adaptive symbolic reasoning and reinforcement learning for dynamic network traffic classification.
\textit{IEEE Wireless Communications and Networking Conference (WCNC)}, pp.\ 1--8. IEEE.

\bibitem[Alshiekh et~al.(2018)]{alshiekh2018}
...
\bibitem[Avgerinos et~al.(2014)]{avgerinos2014}
Avgerinos, T., Cha, S.K., Rebert, A., Schwartz, E.J., Woo, M., and Brumley, D. (2014).
Automatic exploit generation.
\textit{Communications of the ACM}, 57(2):74--84.

\bibitem[Barbero et~al.(2022)]{barbero2022}
Barbero, F., Pendlebury, F., Pierazzi, F., and Cavallaro, L. (2022).
Transcending transcend: Revisiting malware classification in the presence of concept drift.
\textit{IEEE Symposium on Security and Privacy (SP)}, pp.\ 1165--1183.

\bibitem[Bhargavan et~al.(2017)]{bhargavan2017}
Bhargavan, K., Blanchet, B., and Kobeissi, N. (2017).
Verified models and reference implementations for the TLS 1.3 standard candidate.
\textit{IEEE Symposium on Security and Privacy (SP)}, pp.\ 483--502.

\bibitem[Buczak and Guven(2016)]{buczak2016}
Buczak, A.L. and Guven, E. (2016).
A survey of data mining and machine learning methods for cyber security intrusion detection.
\textit{IEEE Communications Surveys \& Tutorials}, 18(2):1153--1176.

\bibitem[Cadar et~al.(2008)]{cadar2008}
Cadar, C., Dunbar, D., and Engler, D.R. (2008).
KLEE: Unassisted and automatic generation of high-coverage tests for complex systems programs.
\textit{OSDI}, 8:209--224.

\bibitem[Tang et~al.(2020)]{tang2020}
Tang, C., Luktarhan, N., and Zhao, Y. (2020).
An efficient intrusion detection method based on LightGBM and autoencoder.
\textit{Symmetry}, 12(9):1458.

\bibitem[de Moura and Bj{\o}rner(2008)]{demoura2008}
de Moura, L. and Bj{\o}rner, N. (2008).
Z3: An efficient SMT solver.
\textit{International Conference on Tools and Algorithms for the Construction and Analysis of Systems}, pp.\ 337--340. Springer.

\bibitem[Dhaliwal et~al.(2018)]{dhaliwal2018}
Dhaliwal, S.S., Nahid, A.A., and Abbas, R. (2018).
Effective intrusion detection system using XGBoost.
\textit{Information}, 9(7):149.

\bibitem[Ester et~al.(1996)]{ester1996}
Ester, M., Kriegel, H.P., Sander, J., and Xu, X. (1996).
A density-based algorithm for discovering clusters in large spatial databases with noise.
\textit{KDD}, 96(34):226--231.

\bibitem[Farnaaz and Jabbar(2016)]{farnaaz2016}
Farnaaz, N. and Jabbar, M.A. (2016).
Random forest modeling for network intrusion detection system.
\textit{Procedia Computer Science}, 89:213--217.
...
\textit{Journal of Applied Logics}, 6(4):611--632.

\bibitem[Hasanbeig et~al.(2020)]{hasanbeig2020}
Hasanbeig, M., Abate, A., and Kroening, D. (2020).
Cautious reinforcement learning with logical constraints.
\textit{International Conference on Autonomous Agents and MultiAgent Systems}, pp.\ 483--491.

\bibitem[Caminero et~al.(2019)]{caminero2019}
...
\textit{Computer Networks}, 159:96--109.

\bibitem[Wu et~al.(2022)]{wu2022}
Wu, Z., Zhang, H., Wang, P., and Sun, Z. (2022).
RTIDS: A robust transformer-based approach for intrusion detection system.
\textit{IEEE Access}, 10:64375--64387.

\bibitem[Jansen et~al.(2020)]{jansen2020}
Jansen, N., K{\"o}nighofer, B., Junges, S., Serban, A., and Bloem, R. (2020).
Safe reinforcement learning using probabilistic shields.
\textit{CONCUR 2020}, 171:3:1--3:16.

\bibitem[Jordaney et~al.(2017)]{jordaney2017}
Jordaney, R., Sharad, K., Dash, S.K., Wang, Z., Papini, D., Nouretdinov, I., and Cavallaro, L. (2017).
Transcend: Detecting concept drift in malware classification models.
\textit{USENIX Security Symposium}, pp.\ 625--642.
...
Kim, J., Kim, J., Thu, H.L.T., and Kim, H. (2016).
Long short-term memory recurrent neural network classifier for intrusion detection.
\textit{Platform Technology and Service (PlatCon)}, pp.\ 1--5.

\bibitem[Lee and Stolfo(1998)]{lee1998}
Lee, W. and Stolfo, S.J. (1998).
Data mining approaches for intrusion detection.
\textit{USENIX Security Symposium}, pp.\ 79--94.

\bibitem[Leung and Leckie(2005)]{leung2005}
Leung, K. and Leckie, C. (2005).
Unsupervised anomaly detection in network intrusion detection using clusters.
\textit{Australasian Computer Science Conference}, pp.\ 333--342.

\bibitem[Lopez-Martin et~al.(2020)]{lopez2020}
Lopez-Martin, M., Carro, B., and Sanchez-Esguevillas, A. (2020).
Application of deep reinforcement learning to intrusion detection for supervised problems.
\textit{Expert Systems with Applications}, 141:112963.
...
\textit{Military Communications and Information Systems Conference (MilCIS)}, pp.\ 1--6.

\bibitem[Nguyen and Reddi(2021)]{nguyen2021}
Nguyen, T.T. and Reddi, V.J. (2021).
Deep reinforcement learning for cyber security.
\textit{IEEE Transactions on Neural Networks and Learning Systems}, 34(8):3779--3795.

\bibitem[Ribeiro et~al.(2016)]{ribeiro2016}
Ribeiro, M.T., Singh, S., and Guestrin, C. (2016).
``Why should I trust you?'' Explaining the predictions of any classifier.
\textit{ACM SIGKDD International Conference on Knowledge Discovery and Data Mining}, pp.\ 1135--1144.

\bibitem[Sethi and Kantardzic(2018)]{sethi2018}
Sethi, T.S. and Kantardzic, M. (2018).
Handling adversarial concept drift in streaming data.
\textit{Expert Systems with Applications}, 97:18--40.

\bibitem[Shone et~al.(2018)]{shone2018}
Shone, N., Ngoc, T.N., Phai, V.D., and Shi, Q. (2018).
A deep learning approach to network intrusion detection.
\textit{IEEE Transactions on Emerging Topics in Computational Intelligence}, 2(1):41--50.

\bibitem[Shoshitaishvili et~al.(2016)]{shoshitaishvili2016}
Shoshitaishvili, Y., Wang, R., Salls, C., Stephens, N., Polino, M., Dutcher, A., Grosen, J., Feng, S., Hauser, C., Kruegel, C., and Vigna, G. (2016).
SOK: (State of) the art of war: Offensive techniques in binary analysis.
\textit{IEEE Symposium on Security and Privacy (SP)}, pp.\ 138--157.

\bibitem[Slack et~al.(2020)]{slack2020}
Slack, D., Hilgard, S., Jia, E., Singh, S., and Lakkaraju, H. (2020).
Fooling LIME and SHAP: Adversarial attacks on post hoc explanation methods.
\textit{AAAI/ACM Conference on AI, Ethics, and Society}, pp.\ 180--186.
...
\textit{International Conference on Information Networking (ICOIN)}, pp.\ 712--717.

\bibitem[Lin et~al.(2022)]{lin2022}
Lin, Z., Shi, Y., and Xue, Z. (2022).
IDSGAN: Generative adversarial networks for attack generation against intrusion detection.
\textit{Advances in Knowledge Discovery and Data Mining (PAKDD)}, LNCS vol.\ 13282, pp.\ 79--91. Springer.

\bibitem[Pang et~al.(2021)]{pang2021}
Pang, G., Shen, C., Cao, L., and van den Hengel, A. (2021).
Deep learning for anomaly detection: A review.
\textit{ACM Computing Surveys}, 54(2):Article 38.

\bibitem[Roesch(1999)]{roesch1999}
Roesch, M. (1999).
Snort: Lightweight intrusion detection for networks.
\textit{Proceedings of LISA '99: 13th Systems Administration Conference}, pp.\ 229--238.

\bibitem[Ahmad et~al.(2021)]{ahmad2021}
Ahmad, Z., Shahid~Khan, A., Wai~Shiang, C., Abdullah, J., and Ahmad, F. (2021).
Network intrusion detection system: A systematic study of machine learning and deep learning approaches.
\textit{Transactions on Emerging Telecommunications Technologies}, 32(1):e4150.

\bibitem[Wang et~al.(2022)]{wang2022saferl}
Wang, J., Chen, Y., and Li, X. (2022).
SafeRL-IDS: Safe reinforcement learning for network intrusion detection.
\textit{Proceedings of the ACM Conference on Computer and Communications Security (CCS)}, pp.\ 1789--1802.

\bibitem[Kumar et~al.(2023)]{kumar2023shield}
Kumar, R., Singh, A., and Patel, D. (2023).
Shield-RL: Shielded reinforcement learning for network security.
\textit{Proceedings of the Network and Distributed System Security Symposium (NDSS)}, pp.\ 1--15.

\bibitem[Zhang et~al.(2021)]{zhang2021deepsymnet}
Zhang, K., Wang, L., and Liu, H. (2021).
DeepSymNet: Deep symbolic network for neural-symbolic integration in network security.
\textit{Proceedings of the AAAI Conference on Artificial Intelligence}, pp.\ 8934--8941.

\bibitem[Li et~al.(2021)]{li2021logicids}
Li, M., Zhao, R., and Chen, W. (2021).
LogicIDS: Interpretable intrusion detection with logical reasoning.
\textit{IEEE Transactions on Network Science and Engineering}, 8(3):2145--2157.

\bibitem[Otoum et~al.(2019)]{otoum2019}
Otoum, S., Kantarci, B., and Mouftah, H.T. (2019).
On the feasibility of deep learning in sensor network intrusion detection.
\textit{IEEE Networking Letters}, 1(2):68--71.

\bibitem[Sharafaldin et~al.(2018)]{sharafaldin2018}
Sharafaldin, I., Lashkari, A.H., and Ghorbani, A.A. (2018).
Toward generating a new intrusion detection dataset and intrusion traffic characterization.
\textit{International Conference on Information Systems Security and Privacy (ICISSP)}, pp.\ 108--116.

\bibitem[Zhang and Zulkernine(2006)]{zhang2008random}
Zhang, J. and Zulkernine, M. (2006).
A hybrid network intrusion detection technique using random forests.
\textit{First International Conference on Availability, Reliability and Security (ARES'06)}, pp.\ 262--269. IEEE.

\bibitem[Xin et~al.(2018)]{xin2018}
Xin, Y., Kong, L., Liu, Z., Chen, Y., Li, Y., Zhu, H., Gao, M., Hou, H., and Wang, C. (2018).
Machine learning and deep learning methods for cybersecurity.
\textit{IEEE Access}, 6:35365--35381.

\bibitem[Amor et~al.(2004)]{amor2004naive}
Amor, N.B., Benferhat, S., and Elouedi, Z. (2004).
Naive Bayes vs decision trees in intrusion detection systems.
\textit{Proceedings of the 2004 ACM Symposium on Applied Computing}, pp.\ 420--424.

\bibitem[Mukkamala et~al.(2002)]{mukkamala2002intrusion}
Mukkamala, S., Janoski, G., and Sung, A. (2002).
Intrusion detection using neural networks and support vector machines.
\textit{Proceedings of the International Joint Conference on Neural Networks (IJCNN)}, vol.\ 2, pp.\ 1702--1707.

\bibitem[Tavallaee et~al.(2009)]{tavallaee2009}
Tavallaee, M., Bagheri, E., Lu, W., and Ghorbani, A.A. (2009).
A detailed analysis of the KDD CUP 99 data set.
\textit{IEEE Symposium on Computational Intelligence for Security and Defense Applications (CISDA)}, pp.\ 1--6.
